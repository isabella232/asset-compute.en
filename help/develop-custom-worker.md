...
Add the API services to the [!DNL Asset Compute] Console workspace created in [set up](#set-up) step 5. These services will be part of the JWT access token generated by [!DNL Asset Compute Service]. The token and other credentials will be accessible inside the worker action `params` object.
